		...
		<div>
			...
			<Accordion type="single" collapsible className="w-full relative">
				{chapterData.map((chapter) => (
					<AccordionItem
						value={`item-${chapter.chapter}`}
						className="border-b relative"
					>
						{/* Adding a status badge to let user know what is still pending. */}
						{chapter.status == "Pending" ? (
							<Badge
								variant="secondary"
								className="absolute right-2 md:right-10 top-5 md:top-10 text-md md:text-2xl z-10 bg-primary"
							>
							...
							<Badge
								variant="destructive"
								className="absolute right-2 md:right-10 top-5 md:top-10 text-md md:text-2xl z-10"
							>
								{chapter.status.toUpperCase()}
							</Badge>
						)}
						<AccordionTrigger
							...
							className="rounded-none border-b text-accent hover:bg-accent-foreground px-5 md:px-20 flex items-center"
						>
							<h2 className="font-black text-4xl lg:text-8xl text-secondary-foreground tracking-tighter">
								{chapter.title.toUpperCase()}
							</h2>
						</AccordionTrigger>
						<AccordionContent className="px-10 md:px-25 py-10 bg-background">
							<div className="md:w-1/2">
								<h3 className="text-3xl md:text-4xl font-black mb-5 ">
									{chapter.subTitle}
								</h3>
								<p className="text-lg md:text-2xl mb-6">{chapter.summary}</p>
